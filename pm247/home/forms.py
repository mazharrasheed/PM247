...
class EngineerAvailabilityForm(forms.ModelForm):
    jobs = forms.ModelMultipleChoiceField(queryset=Job_Type.objects.all(), widget=forms.SelectMultiple(attrs={'class': 'form-select selectpicker'}))
    cities = forms.ModelMultipleChoiceField(queryset=Post_Code.objects.all(), widget=forms.SelectMultiple(attrs={'class': 'form-select selectpicker'}))
